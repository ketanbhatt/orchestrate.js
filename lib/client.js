...
/*
Declare cache with default time to live = 10s
Reference to original response is sent to the cache ( useClones: false ),
this is to support complex objects like promises.
 */
var NodeCache = require( "node-cache" );
var myCache = new NodeCache({ stdTTL: 3600, useClones: false });
var useCache = false
 ...
 */
function Client (token, apiEndpoint, cache) {
  assert(token, 'API key required.');
  if(cache == true) useCache = true
...
Client.prototype._get = function (url) {
  if(useCache) {
    // for every get request, checking availability first in the cache
    // if not found, getting the response and storing in cache for future use
    value = myCache.get(url)
    if ( value == undefined ){
      var response_promise = this._request('GET', url)
      success = myCache.set(url, response_promise)
...
Client.prototype._post = function (url, data, header) {
  // Emptying cache on all requests that can change data in the database
  // This flushing can be implemented more smartly to only delete updated content
  if(useCache) myCache.flushAll()
